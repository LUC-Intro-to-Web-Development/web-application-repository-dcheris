/**
 * eloquent-ffmpeg v0.13.0-alpha.0 — git 3808291d880e
...
const toReadable = (source) => (isReadableStream(source) ? source : stream.Readable.from(source instanceof Uint8Array ? [source] : source, { objectMode: false }));
...
async function getLines(ffmpegPath, args) {
    const process = child_process.spawn(ffmpegPath, args, { stdio: 'pipe' });
...
async function probe(source, options = {}) {
    const { probeSize, analyzeDuration, ffprobePath = getFFprobePath(), logLevel = exports.LogLevel.Error, format, args = [], spawnOptions = {}, } = options;
    ...
    const extractError = async (error) => {
        const logs = [];
        if (stderr.readable)
            for await (const line of readline.createInterface(stderr)) {
                logs.push(line);
            }
...
class Process {
    ...
    async *progress() {
        let progress = {};
        for await (const line of readline.createInterface(__classPrivateFieldGet(this, _process).stdout)) {
            try {
        ...
        return new Promise((resolve, reject) => {
            ...
            const error = async () => {
                if (!__classPrivateFieldGet(this, _error)) {
                    const stderr = ffmpeg.stderr.readable ? await fromAsyncIterable(readline.createInterface(ffmpeg.stderr)) : [];
...
class Command {
    constructor(options) {
        ...
        this.logLevel = (_a = options.logLevel) !== null && _a !== void 0 ? _a : exports.LogLevel.Error;
    ...
    concat(sources, options = {}) {
        ...
        const stream$1 = stream.Readable.from([Buffer.from(directives, 'utf8')], { objectMode: false });
        const path = getSocketPath();
        inputStreams.push([path, stream$1]);
        const input = new Input(getSocketUrl(path), true, stream$1);
...
exports.FFprobeError = FFprobeError;
exports.ffmpeg = ffmpeg;
exports.getCodecs = getCodecs;
exports.getDecoders = getDecoders;
exports.getDemuxers = getDemuxers;
...
exports.getMuxers = getMuxers;
exports.getPixelFormats = getPixelFormats;
exports.getVersion = getVersion;
exports.probe = probe;
exports.setFFmpegPath = setFFmpegPath;
exports.setFFprobePath = setFFprobePath;
exports.spawn = spawn;
